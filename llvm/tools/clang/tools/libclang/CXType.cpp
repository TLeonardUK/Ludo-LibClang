...
#if 1 // LD_MODIFICATION

CXString clang_getTypeFullyQualifiedSpelling(CXType CT) {
	QualType T = GetQualType(CT);
	if (T.isNull())
		return cxstring::createEmpty();

	CXTranslationUnit TU = GetTU(CT);
	std::string FullyQualified = TypeName::getFullyQualifiedName(T, cxtu::getASTUnit(TU)->getASTContext());

	return cxstring::createDup(FullyQualified.c_str());
}

#endif // LD_MODIFICATION
